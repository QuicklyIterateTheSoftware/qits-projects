...
/**
 * {@code /projects} → {@code /projects/}, and nothing else.
 *
 * <p>Quinoa mounts the web client at {@code /projects/*}, which does not match the bare segment — so
 * before this route existed, typing {@code /projects} into a browser answered 404 while
 * {@code /projects/} served the client. Upstream behaviour, but not a defensible surface: the segment
...
public class WebUiRedirect {
  ...
  void init(@Observes Router router) {
    router
        ...
        .handler(
            rc -> {
              // Vert.x path routes are trailing-slash tolerant: route("/projects") matches /projects/ too,
              ...
              if (!"/projects".equals(rc.request().path())) {
                rc.next();
                return;
              }
              ...
              rc.response()
                  .setStatusCode(301)
                  .putHeader("Location", query == null ? "/projects/" : "/projects/?" + query)
                  .end();
            });
  }
}
