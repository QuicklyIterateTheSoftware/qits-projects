 ...
 *
 * <p><b>A per-repository copy</b>, and deliberately one: the client jar's own stub is package-private
 * in a jar this repository consumes, qits-ci carries an identical copy for the same reason, and a
 * test-jar to bridge sixty lines would couple this suite to another repository's fixtures. The JDK
 * server rather than Vert.x, so no web stack has to be dragged in to prove a header went out.
 *
