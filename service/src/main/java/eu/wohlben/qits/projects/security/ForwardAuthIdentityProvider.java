 ...
 * single global check (`qits.auth.required-role`) performed at qits-gateway, which therefore emits
 * no groups header — one fewer trusted header, and it keeps "the gateway terminates auth entirely"
 * literally true. No code in this service makes a role decision, so roles here would be a security
 ...
 *
 * <p>The principal is the <b>name</b>, not the stable subject id, because it is written into the
 * epics {@code changed_by} audit column and the existing rows hold usernames — making the id the
 * principal would leave that column holding two incompatible kinds of value with nothing to tell
 * them apart.
 */
@ApplicationScoped
