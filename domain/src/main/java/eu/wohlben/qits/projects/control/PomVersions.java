 ...
 *
 * A port of qits-workspaces-service's class of the same name, unchanged but for the exception type.
 * Both services now stamp release versions and both must render one instant into one set of bytes;
 * the duplication is the standing ruling about vocabulary jars this platform's Maven registry does
 * not serve, and the measurements above are that repository's, made against all 45 poms of this
 * platform. Do not "simplify" the line/column arithmetic back to {@code getCharacterOffset()}.
 */
public final class PomVersions {
    ...
    try {
      while (reader.hasNext()) {
        ...
        if (name.equals("dependency")) {
          if (inDependency) {
            throw new ManifestBumpException("nested <dependency> in " + origin);
          }
          inDependency = true;
          continue;
        }
      ...
      throw new ManifestBumpException("cannot parse " + origin + ": " + e.getMessage(), e);
    } finally {
      close(reader);
    }

    if (artifactId == null) {
      throw new ManifestBumpException(origin + " declares no <artifactId>");
    }
  ...
  private static String pop(List<String> path, String origin) {
    if (path.isEmpty()) {
      throw new ManifestBumpException("unbalanced elements in " + origin);
    }
