...
class WebUiRedirectTest {
  ...
  void theBareSegmentRedirectsToTheClient() {
    given()
        ...
        .then()
        .statusCode(301)
        .header("Location", equalTo("/projects/"));
  }
  ...
  void theQueryStringTravels() {
    given()
        ...
        .then()
        .statusCode(301)
        .header("Location", equalTo("/projects/?x=y"));
  }
  ...
  void theSlashFormIsNotThisRoutesBusiness() {
    ...
    // redirect would answer /projects/ too — sitting AHEAD of Quinoa and looping onto itself. With
    // Quinoa off under %test the slash form falls through to a plain 404, which is exactly the
    // proof: this route let it pass.
    given().redirects().follow(false).when().get("/projects/").then().statusCode(404);
  }
  ...
  void aWriteToTheBareSegmentIsMethodNotAllowed() {
    ...
    given().redirects().follow(false).when().post("/projects").then().statusCode(405);
  }
}
