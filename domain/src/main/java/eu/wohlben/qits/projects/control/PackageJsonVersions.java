 ...
 *
 * <p>A port of qits-workspaces-service's class of the same name, unchanged but for the exception
 * type — see {@link PomVersions} for why the duplication rather than a jar.
 */
public final class PackageJsonVersions {
   ...
   *
   * @throws ManifestBumpException if the document is not well-formed JSON, or if a requested pointer
    ...
    try (JsonParser parser = new JsonFactory().createParser(json)) {
      ...
      while ((token = parser.nextToken()) != null) {
        ...
        if (found.putIfAbsent(pointer, new TextSplice.Span(start, end)) != null) {
          throw new ManifestBumpException(origin + " holds " + pointer + " more than once");
        }
      }
    } catch (IOException e) {
